...
ExpertParallelDomain::ExpertParallelDomain(
...
ExpertRange ExpertParallelDomain::expert_range(
...
std::uint64_t ExpertParallelDomain::owner(
...
ExpertParallelDomain::partition(
    const std::vector<std::uint64_t>& global_counts) const {
  if (global_counts.size() !=
      static_cast<std::size_t>(total_experts_)) {
    throw ParallelDomainError(
