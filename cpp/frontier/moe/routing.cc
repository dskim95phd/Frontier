...
std::array<std::uint64_t, 4> seed_sequence_state(
    std::uint64_t seed) {
  ...
  for (std::size_t i = 0; i < pool.size(); ++i) {
    pool[i] = hashmix(i < entropy.size() ? entropy[i] : 0U,
                      hash_constant);
  }
  ...
  for (std::size_t source = pool.size();
       source < entropy.size();
       ++source) {
    for (std::size_t destination = 0;
         destination < pool.size();
      ...
      pool[destination] = mix(
          pool[destination],
          hashmix(entropy[source], hash_constant));
    }
  }
...
Uint128 add128(Uint128 lhs, Uint128 rhs) {
  ...
  return Uint128{
      lhs.high + rhs.high + static_cast<std::uint64_t>(low < rhs.low),
      low};
}

std::pair<std::uint64_t, std::uint64_t> multiply64(
    std::uint64_t lhs,
    std::uint64_t rhs) {
...
class NumpyPcg64 {
 public:
  explicit NumpyPcg64(std::uint64_t seed) {
    ...
    seed_state(
        Uint128{generated[0], generated[1]},
...
std::vector<double> distribution_weights(
  ...
  std::vector<double> weights(
      static_cast<std::size_t>(experts), 1.0);
  switch (distribution) {
    ...
    case config::MoeRoutingDistribution::kSkewed:
      for (std::size_t i = 0; i < weights.size(); ++i) {
        weights[i] =
            1.0 / std::pow(static_cast<double>(i + 1), 0.35);
      }
      break;
...
std::vector<std::uint64_t> discretize_expert_weights(
  ...
  for (std::size_t i = 0; i < weights.size(); ++i) {
    ...
    const double exact =
        static_cast<double>(total_tokens) * normalized[i];
  ...
  while (allocated < total_tokens) {
    ++result[order[static_cast<std::size_t>(
        remainder_index % order.size())]];
    ++allocated;
    ++remainder_index;
  }
  return result;
...
RoutingAllocation route_tokens(
  ...
  std::vector<std::uint64_t> counts(
      static_cast<std::size_t>(total_experts), 0);
  ...
  if (config.mode == config::MoeRoutingMode::kUniformLegacy) {
    ...
    for (std::uint64_t expert = 0; expert < total_experts; ++expert) {
      ...
      config.mode == config::MoeRoutingMode::kUniformRandom) {
  ...
  return RoutingAllocation{
      ...
      .global_expert_tokens = counts,
      .lane_expert_tokens = domain.partition(counts),
  };
}
