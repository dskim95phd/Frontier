...
Batch::Batch(BatchId batch_id, IterationId iteration_id,
             std::vector<RequestBatchSnapshot> requests, SimTime scheduled_at,
             Generation schedule_epoch)
    : Batch(batch_id, iteration_id, std::move(requests), scheduled_at,
            schedule_epoch, ReplicaId{0}, DataParallelId{0}, 1) {}

Batch::Batch(BatchId batch_id, IterationId iteration_id,
             std::vector<RequestBatchSnapshot> requests, SimTime scheduled_at,
             Generation schedule_epoch, ReplicaId replica_id,
             DataParallelId dp_id, std::uint64_t num_pipeline_stages,
             ClusterType cluster_type, BatchKind kind,
             config::ModelKind model_kind, MoESyncGroupId moe_sync_group_id,
             MoEParticipantId moe_participant_id)
    : batch_id_(batch_id), iteration_id_(iteration_id),
      requests_(std::move(requests)), scheduled_at_(scheduled_at),
      schedule_epoch_(schedule_epoch), replica_id_(replica_id), dp_id_(dp_id),
      num_pipeline_stages_(num_pipeline_stages), cluster_type_(cluster_type),
      kind_(kind), model_kind_(model_kind),
    ...
    if (kind_ == BatchKind::kMoeIdle &&
        (!requests_.empty() || model_kind_ != config::ModelKind::kMoe ||
         !moe_sync_group_id_.valid() || !moe_participant_id_.valid())) {
        throw BatchError("idle MoE batch requires no requests and valid "
                         "synchronization IDs");
    }
...
void Batch::reset_stage_layer() { stage_layer_ = LayerId{0}; }

void Batch::set_moe_synchronization(MoESyncGroupId sync_group_id,
                                    MoEParticipantId participant_id) {
    if (!is_moe() || !sync_group_id.valid() || !participant_id.valid()) {
        throw BatchError(
            "MoE synchronization requires a MoE batch and valid IDs");
    }
    if ((moe_sync_group_id_.valid() && moe_sync_group_id_ != sync_group_id) ||
