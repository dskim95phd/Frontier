...
class KVCacheTransferInfo {
  public:
    KVCacheTransferInfo(TransferId transfer_id, RequestId request_id,
                        BatchId source_batch_id, ReplicaId source_replica_id,
                        DataParallelId source_dp_id, std::uint64_t size_bytes,
                        double predicted_time_ms, Generation source_generation);

    [[nodiscard]] TransferId id() const noexcept { return transfer_id_; }
    ...
    [[nodiscard]] KVCacheTransferState state() const noexcept { return state_; }
