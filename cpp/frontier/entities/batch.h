...
struct RequestBatchSnapshot {
    ...
    friend bool operator==(const RequestBatchSnapshot &lhs,
                           const RequestBatchSnapshot &rhs) {
        return std::tie(lhs.request_id, lhs.scheduled_tokens, lhs.runtime_epoch,
                        lhs.execution_epoch, lhs.processed_tokens,
                        lhs.scheduler_frontier) ==
               std::tie(rhs.request_id, rhs.scheduled_tokens, rhs.runtime_epoch,
                        rhs.execution_epoch, rhs.processed_tokens,
                        rhs.scheduler_frontier);
    }
};
...
class Batch {
  public:
    Batch(BatchId batch_id, IterationId iteration_id,
          std::vector<RequestBatchSnapshot> requests, SimTime scheduled_at,
          Generation schedule_epoch);
    Batch(BatchId batch_id, IterationId iteration_id,
          std::vector<RequestBatchSnapshot> requests, SimTime scheduled_at,
          Generation schedule_epoch, ReplicaId replica_id, DataParallelId dp_id,
    ...
    [[nodiscard]] const std::vector<RequestBatchSnapshot> &
    ...
    [[nodiscard]] LayerId stage_layer() const noexcept { return stage_layer_; }
    ...
    void set_moe_synchronization(MoESyncGroupId sync_group_id,
