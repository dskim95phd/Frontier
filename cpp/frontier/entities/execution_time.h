...
struct ExecutionTime {
    ...
    [[nodiscard]] double total_ms() const noexcept {
        return dense_compute_ms + tp_communication_ms + pp_communication_ms +
               moe_gating_linear_ms + moe_gating_routing_topk_ms +
               moe_grouped_gemm_ms + moe_shuffling_ms +
               moe_post_attention_norm_ms + moe_tp_communication_ms +
               ep_dispatch_ms + ep_combine_ms + dp_input_communication_ms +
               dp_output_communication_ms + synchronization_wait_ms;
    }

    friend bool operator==(const ExecutionTime &lhs, const ExecutionTime &rhs) {
        return std::tie(lhs.dense_compute_ms, lhs.tp_communication_ms,
                        lhs.pp_communication_ms, lhs.moe_gating_linear_ms,
               ...
               std::tie(rhs.dense_compute_ms, rhs.tp_communication_ms,
                        rhs.pp_communication_ms, rhs.moe_gating_linear_ms,
