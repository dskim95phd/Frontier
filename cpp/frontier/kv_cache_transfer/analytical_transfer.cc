...
std::uint64_t dense_kv_cache_size_bytes(
    std::uint64_t num_tokens,
    const DenseKvLayout& layout) {
  if (layout.num_layers == 0 ||
      layout.num_kv_heads_per_worker == 0 ||
      layout.head_dim == 0 ||
      layout.kv_factor == 0) {
    throw TransferModelError(
        "dense KV layout dimensions must be positive");
  }
...
TransferPrediction predict_transfer(
  ...
  return TransferPrediction{
      ...
      .effective_size_bytes = effective_size,
      .transfer_time_ms =
