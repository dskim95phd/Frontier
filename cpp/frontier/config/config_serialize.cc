...
OrderedJson serialize_single_cluster(
    const ClusterRuntimeConfig& cluster) {
  return OrderedJson::object({
...
std::string serialize_simulation_config_json(
  ...
  if (config.system_architecture ==
    ...
    if (config.system_architecture !=
        ...
        !std::holds_alternative<PddRuntimeConfig>(
            config.runtime)) {
      throw ConfigError(
    ...
    root["clusters"] =
        serialize_pdd_clusters(runtime.clusters);
    root["kv_cache_transfer"] =
        serialize_kv_cache_transfer(runtime.kv_cache_transfer);
    return root.dump(2) + '\n';
  }
  if (!std::holds_alternative<ClusterRuntimeConfig>(
          config.runtime)) {
    throw ConfigError(
        "co-location config requires a monolithic cluster");
  }
