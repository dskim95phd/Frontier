...
class EventQueue {
  public:
    ...
    EventSequence push(SimTime time, Payload payload) {
        return push_payload(time, EventPayload{std::move(payload)});
    }
  ...
  private:
    EventSequence push_payload(SimTime time, EventPayload payload);

    struct LaterEvent {
        [[nodiscard]] bool operator()(const Event &left,
