...
KvBlockAccounting::KvBlockAccounting(const config::SchedulerConfig &config)
    : block_size_(config.block_size), num_blocks_(config.num_blocks),
      watermark_blocks_(
          static_cast<std::uint64_t>(config.watermark_blocks_fraction *
...
std::uint64_t KvBlockAccounting::additional_blocks_required(
    ...
    if (allocation->second >
        ...
        throw KvBlockAccountingError(
            "KV reserved token count overflows uint64");
    }
    const std::uint64_t reserved_tokens = allocation->second * block_size_;
...
bool KvBlockAccounting::can_reserve(RequestId request_id,
...
void KvBlockAccounting::reserve(RequestId request_id,
                                std::uint64_t kv_accounted_tokens,
...
std::uint64_t
KvBlockAccounting::allocated_blocks(RequestId request_id) const noexcept {
    const auto allocation = allocations_.find(request_id);
