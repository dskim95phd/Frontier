...
std::vector<ClusterRequestAssignment> KvAwareClusterScheduler::schedule() {
    ...
    for (const QueuedRequest &queued : request_queue_) {
        ...
        const std::uint64_t current_tokens = std::max(
            {incoming.num_prefill_tokens(), incoming.num_processed_tokens(),
        ...
        for (std::size_t target_index = 0;
            ...
            const std::uint64_t block_size = target.kv_block_size();
            const std::uint64_t incoming_blocks =
                block_size == 0
                    ? 0
                    : current_tokens / block_size +
                          (current_tokens % block_size == 0 ? 0 : 1);
            const std::uint64_t queued_blocks = target.queued_kv_blocks();
