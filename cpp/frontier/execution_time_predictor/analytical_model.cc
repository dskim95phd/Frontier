...
void validate_efficiency(const Efficiency& efficiency) {
  for (const auto& [value, name] : {
    ...
    if (!std::isfinite(value) || value <= 0.0 || value > 1.0) {
      throw AnalyticalModelError(
          std::string{name} + " must satisfy 0 < value <= 1");
    }
  }
  if (!std::isfinite(efficiency.overlap_penalty) ||
      efficiency.overlap_penalty < 0.0 ||
      efficiency.overlap_penalty > 1.0) {
    throw AnalyticalModelError(
...
double DenseLayerTimes::total_ms() const noexcept {
  return attention_pre_projection_ms +
...
RooflineResult predict_roofline(
    const DeviceCeilings& device,
    Precision precision,
    const KernelWork& work,
    const Efficiency& efficiency,
  ...
  if (!std::isfinite(device.hbm_bandwidth_tbps) ||
    ...
    throw AnalyticalModelError(
        "HBM bandwidth must be finite and positive");
  }
  ...
  if (work.flops == 0.0 && work.hbm_bytes == 0.0) {
    return RooflineResult{
        ...
        .bottleneck = Bottleneck::kNone,
    };
  }

  const double compute_time_ms =
      work.flops /
      (peak_tflops(device, precision) * 1e12 * efficiency.compute) *
      1e3;
  const double memory_time_ms =
      work.hbm_bytes /
      (device.hbm_bandwidth_tbps * 1e12 * efficiency.memory) *
      1e3;
  ...
  const double predicted_time_ms =
      launch_time_ms + maximum +
      efficiency.overlap_penalty * minimum;
...
KernelWork gemm_work(
  ...
  if (m == 0 || k == 0 || n == 0) {
    return KernelWork{.flops = 0.0, .hbm_bytes = 0.0};
  }
  ...
  return KernelWork{
      .flops =
          2.0 * resolved_m * resolved_k * resolved_n * multiplier,
      .hbm_bytes =
          element_bytes *
          (resolved_m * resolved_k +
           multiplier * resolved_k * resolved_n +
           multiplier * resolved_m * resolved_n),
  };
}

KernelWork streaming_work(
  ...
  return KernelWork{
      .flops = flops,
      .hbm_bytes = (elements_read + elements_written) * element_bytes,
  };
}

KernelWork attention_context_work(
    ...
    double element_bytes) {
  if (local_query_heads == 0 || local_kv_heads == 0 ||
      head_dim == 0) {
    throw AnalyticalModelError(
  ...
  for (const AttentionRequestSlice& request : requests) {
    const double query_tokens =
        static_cast<double>(request.query_tokens);
    const double past_context =
        static_cast<double>(request.past_context);
    const double average_visible_kv =
        past_context + (query_tokens + 1.0) / 2.0;
    total_flops +=
        4.0 * static_cast<double>(local_query_heads) *
        static_cast<double>(head_dim) * query_tokens *
        average_visible_kv;
    const double q_and_output =
        2.0 * query_tokens *
        static_cast<double>(local_query_heads) *
        static_cast<double>(head_dim);
    const double new_kv_input =
        2.0 * query_tokens * static_cast<double>(local_kv_heads) *
        static_cast<double>(head_dim);
    const double cached_kv_reads =
        2.0 * past_context * static_cast<double>(local_kv_heads) *
        static_cast<double>(head_dim);
    total_elements += q_and_output + new_kv_input + cached_kv_reads;
  }
  return KernelWork{
      .flops = total_flops,
      .hbm_bytes = total_elements * element_bytes,
  };
}

DenseLayerTimes predict_dense_layer(
    const DeviceCeilings& device,
  ...
  const double intermediate =
      static_cast<double>(local_intermediate);

  const KernelWork prefill_attention = attention_context_work(
      batch.prefill_requests,
      local_query_heads,
      local_kv_heads,
      model.head_dim,
      element_bytes);
  const KernelWork decode_attention = attention_context_work(
      batch.decode_requests,
      local_query_heads,
      local_kv_heads,
      model.head_dim,
  ...
  const double rope_elements =
      tokens * (query_heads + kv_heads) * head_dim;
  const double kv_elements = tokens * 2.0 * kv_heads * head_dim;
  const double attention_norm_factor =
      model.fused_add_norm ? 3.0 : 2.0;
  const double mlp_norm_factor =
      model.fused_add_norm ? 3.0 : 2.0;
  ...
  if (!model.fused_add_norm) {
    ...
    residual_add_ms = kernel_ms(
        streaming_work(
            2.0 * residual_elements,
            residual_elements,
            residual_elements,
            element_bytes),
        config.streaming);
  ...
  return DenseLayerTimes{
      ...
      .kv_cache_save_ms = kernel_ms(
          streaming_work(
              ...
              element_bytes),
          config.streaming),
      .attention_norm_ms = kernel_ms(
          streaming_work(
              tokens * hidden * (attention_norm_factor - 1.0),
              tokens * hidden,
              5.0 * tokens * hidden,
              element_bytes),
          config.streaming),
      .prefill_attention_ms = kernel_ms(
          prefill_attention,
          config.prefill_attention),
      .decode_attention_ms = kernel_ms(
          decode_attention,
          config.decode_attention),
      .mlp_up_projection_ms = kernel_ms(
          gemm_work(
              batch.total_tokens,
              model.hidden_size,
              local_intermediate,
              ...
              gated_multiplier),
          gemm_efficiency),
      .mlp_activation_ms = kernel_ms(
          streaming_work(
              activation_elements *
                  static_cast<double>(gated_multiplier),
              activation_elements,
              8.0 * activation_elements,
              element_bytes),
          config.streaming),
      .mlp_down_projection_ms = kernel_ms(
          gemm_work(
              batch.total_tokens,
              local_intermediate,
              model.hidden_size,
              element_bytes),
          gemm_efficiency),
      .mlp_norm_ms = kernel_ms(
          streaming_work(
              tokens * hidden * (mlp_norm_factor - 1.0),
              tokens * hidden,
              5.0 * tokens * hidden,
              element_bytes),
          config.streaming),
      .residual_add_ms = residual_add_ms,
  };
}
