...
void test_directional_serialization_and_full_duplex() {
    ...
    const auto d2h_second = engine.schedule(
        CpuTransferDirection::kD2H, 125'000'000, SimTime::from_seconds(0.0));
    const auto h2d = engine.schedule(CpuTransferDirection::kH2D, 250'000'000,
    ...
    const auto h2d_second = engine.schedule(CpuTransferDirection::kH2D, 1,
                                            SimTime::from_seconds(0.0));
...
void test_zero_bytes_validation_and_large_values() {
    ...
    expect(zero.service_time_ms == 0.25 && zero.completed_at > zero.started_at,
           "zero-byte transfer contract must charge fixed latency");
    const auto large = engine.schedule(
        CpuTransferDirection::kH2D, std::numeric_limits<std::uint64_t>::max(),
    ...
    auto invalid = config();
    invalid.d2h_bandwidth_gbps = std::numeric_limits<double>::quiet_NaN();
...
void test_typed_events_and_names() {
    ...
    expect(
        frontier::metrics::to_string(EventType::kCpuKvCacheOffloadStart) ==
                "cpu_kv_cache_offload_start" &&
            frontier::metrics::to_string(EventType::kCpuKvCacheOffloadEnd) ==
                "cpu_kv_cache_offload_end" &&
            frontier::metrics::to_string(EventType::kCpuKvCacheRestoreStart) ==
                "cpu_kv_cache_restore_start" &&
            frontier::metrics::to_string(EventType::kCpuKvCacheRestoreEnd) ==
