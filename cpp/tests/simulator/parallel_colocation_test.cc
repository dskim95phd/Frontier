...
void test_schema_v3_round_trip_and_validation() {
  ...
  expect_throws<ConfigError>(
      [&invalid] {
        static_cast<void>(
            parse_simulation_config_json(invalid));
      },
...
void test_pp4_fill_drain_and_terminal_release() {
  ...
  expect(
      stages == std::set<std::uint64_t>{0, 1, 2, 3},
      "PP4 output must contain every stage");
  expect(
      empty_schedule_iterations > 0,
...
void test_dp_target_local_pressure_and_preemption() {
  ...
  for (const auto& request : output.requests) {
    preemptions_by_dp[request.dp_id.value()] +=
        request.preemption_count;
  }
  expect(
      output.requests.size() == 4 &&
          preemptions_by_dp.size() == 2 &&
          preemptions_by_dp.at(0) > 0 &&
          preemptions_by_dp.at(1) > 0,
...
void test_pipeline_serialization_overlap_and_fixed_timing() {
  ...
  for (const auto& stage : output.batch_stages) {
    const double expected =
        stage.stage_id.value() == 0 ? 1.0 : 3.0;
    expect(
        stage.execution_time.total_ms() == expected,
        "fixed stage latency must select by PP stage");
    by_stage[Key{
        stage.replica_id.value(),
        stage.dp_id.value(),
  ...
  for (auto& [key, records] : by_stage) {
    ...
    for (std::size_t index = 1; index < records.size(); ++index) {
      expect(
          records[index - 1]->completed_at.seconds() <=
  ...
  for (const auto& left : output.batch_stages) {
    for (const auto& right : output.batch_stages) {
      if (left.replica_id != right.replica_id ||
          left.dp_id != right.dp_id ||
          left.stage_id == right.stage_id) {
        continue;
      }
      if (left.started_at.seconds() <
              right.completed_at.seconds() &&
          right.started_at.seconds() <
              left.completed_at.seconds()) {
        found_cross_stage_overlap = true;
      }
    }
  }
  expect(
      found_cross_stage_overlap,
...
void test_analytical_tp_pp_components_and_determinism() {
  ...
  for (const auto& stage : first.batch_stages) {
    expect(
        stage.execution_time.dense_compute_ms > 0.0 &&
            stage.execution_time.tp_communication_ms > 0.0,
...
int main() {
  ...
  failures += frontier::test::run(
      "PP4 fill, drain, and terminal release",
      test_pp4_fill_drain_and_terminal_release);
  failures += frontier::test::run(
      "DP target-local pressure and preemption",
      test_dp_target_local_pressure_and_preemption);
  failures += frontier::test::run(
      "analytical TP/PP components and determinism",
      test_analytical_tp_pp_components_and_determinism);
  failures += frontier::test::run(
      "analytical primary TP/PP matrix",
