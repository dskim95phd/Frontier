...
frontier::config::SimulationConfig load_config(
    const char* name) {
...
auto load_small_workload() {
  return parse_workload_csv(read_text_file(
      kFixtureRoot / "workloads/step3_pdd_small.csv"));
}
...
std::size_t count_events(
  ...
  return static_cast<std::size_t>(std::count_if(
      ...
      [type](const frontier::Event& event) {
...
void test_local_moe_uses_fast_path() {
  const auto output = run_simulation(
      load_config("fixed_moe_local_colocation.json"),
      load_small_workload());
  ...
  expect(
      output.requests.size() == 2 &&
          ...
          count_events(
              output, EventType::kPrefillSyncCollective) == 0 &&
          count_events(output, EventType::kDecodeSync) == 0 &&
          count_events(
              output, EventType::kDecodeSyncCollective) == 0,
  ...
  expect(
      std::all_of(
          ...
          [](const auto& stage) {
            return stage.execution_time.moe_grouped_gemm_ms > 0.0 &&
...
void test_colocation_runs_all_sync_event_families() {
  ...
  expect(
      std::all_of(
          types.begin(), types.end(), [&output](EventType type) {
  ...
  const bool has_decode_pre_idle_event = std::any_of(
      ...
      [](const frontier::Event& event) {
        ...
        const auto& payload =
            event.as<frontier::DecodeSyncPayload>();
        return payload.is_idle &&
            payload.sync_phase ==
                frontier::moe::SyncPhase::kPreMoe;
      });
  const bool has_decode_post_idle_event = std::any_of(
      ...
      [](const frontier::Event& event) {
        ...
        return payload.is_idle &&
            payload.sync_phase ==
                frontier::moe::SyncPhase::kPostMoe;
      });
  expect(
      has_prefill_idle &&
          !has_decode_pre_idle_event &&
...
void test_pdd_moe_preserves_phi_kv_contract() {
  ...
  const auto output = run_simulation(
      load_config("fixed_moe_sequential_pdd.json"), workload);
  expect(
      output.requests.size() == workload.size() &&
          output.kv_cache_transfers.size() == workload.size() &&
          count_events(output, EventType::kPrefillSync) > 0 &&
          count_events(output, EventType::kDecodeSync) > 0,
      "sequential PDD MoE must synchronize both clusters and complete transfers");

  constexpr std::uint64_t kBytesPerPrefillToken =
      32ULL * 4ULL * 128ULL * 2ULL * 2ULL;
  for (std::size_t index = 0; index < workload.size(); ++index) {
    expect(
        output.kv_cache_transfers.at(index).size_bytes ==
            workload.at(index).num_prefill_tokens *
                kBytesPerPrefillToken,
        "Phi KV transfer size must use model layers, KV heads, and head dim");
  }
}
...
int main() {
  ...
  failures += frontier::test::run(
      "local MoE fast path", test_local_moe_uses_fast_path);
  failures += frontier::test::run(
      "co-location MoE synchronization",
      test_colocation_runs_all_sync_event_families);
  failures += frontier::test::run(
      "PDD MoE Phi KV contract",
