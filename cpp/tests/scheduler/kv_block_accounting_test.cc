...
void test_watermark_and_overcommit_are_rejected() {
    ...
    expect(!blocks.can_reserve(RequestId{2}, 0, 4),
           "watermark must keep one block free");
    expect_throws<KvBlockAccountingError>(
        [&blocks] { blocks.reserve(RequestId{2}, 0, 4); },
...
int main() {
    ...
    failures += frontier::test::run("KV watermark and overcommit are rejected",
