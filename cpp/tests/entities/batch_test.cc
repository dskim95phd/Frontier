...
RequestBatchSnapshot snapshot(RequestId id, std::uint64_t tokens) {
  return RequestBatchSnapshot{
      .request_id = id,
      .scheduled_tokens = tokens,
      .runtime_epoch = 0,
      .execution_epoch = 0,
      .processed_tokens = 0,
      .scheduler_frontier = tokens,
  };
}
...
void test_batch_is_immutable_arena_value() {
  Batch batch{
      ...
      Generation{9}};
  expect(
      batch.total_scheduled_tokens() == 6,
      "batch must sum scheduled tokens");
  expect(
      batch.schedule_epoch() == Generation{9},
...
int main() {
  ...
  failures += frontier::test::run(
      "batch is immutable arena value",
      test_batch_is_immutable_arena_value);
  failures += frontier::test::run(
      "invalid batch shape is rejected",
