...
</head><body><h1>Kimi K3 P24/D64 CPU-per-GPU capacity sweep</h1>
...
def generate_reports(
    ...
    *,
    ...
    horizon_s = float(plan.get("simulation_end_time_s", 0) or 0)
    horizon_hours = horizon_s / 3600.0
    horizon_tag = (
        f"{int(round(horizon_hours))}h"
        if horizon_hours > 0 and abs(horizon_hours - round(horizon_hours)) < 1e-9
        else "custom_horizon"
    )
    prefill_lanes = int(plan.get("prefill_gpus", 24) or 24)
